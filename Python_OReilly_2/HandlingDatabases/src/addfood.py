...
Note that the foods are identified by the animal's name and family,
so we have to look up the primary key.
"""
...
for name, family, foods in data:
    ...
    FROM animal WHERE name=%s and family=%s""", (name, family))
    id = cursor.fetchone()[0]
    for food in foods:
        cursor.execute("""
                       INSERT INTO food (anid, feed)
                       VALUES (%s, %s)""", (id, food))
    db.commit()
